...
//rdma_get_cm_event may wait user's message
    while (rdma_get_cm_event(ec, &event) == 0) {
#ifdef _DEBUG
        printf("event_loop in while\n");
#endif
...
#ifdef _DEBUG
            printf("event_loop in while and RDMA_CM_EVENT_ADDR_RESOLVED\n");
#endif
//client first step
...
#ifdef _DEBUG
            printf("event_loop in while and RDMA_CM_EVENT_ROUTE_RESOLVED\n");
#endif
            TEST_NZ(rdma_connect(event_copy.id, &cm_params));

        } else if (event_copy.event == RDMA_CM_EVENT_CONNECT_REQUEST) {
#ifdef _DEBUG
            printf("event_loop in while and RDMA_CM_EVENT_CONNECT_REQUEST\n");
#endif
//server first step
            build_connection(event_copy.id);
...
#ifdef _DEBUG
            printf("event_loop in while and RDMA_CM_EVENT_ESTABLISHED\n");
#endif
//server second step
            if (s_on_connect_cb)
...
#ifdef _DEBUG
            printf("event_loop in while and RDMA_CM_EVENT_DISCONNECTED\n");
#endif
//server last step
            ...
            printf("event_loop in while and unknown event\n");
